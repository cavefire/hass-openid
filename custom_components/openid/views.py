...
class OpenIDSessionView(HomeAssistantView):
    ...
    requires_auth = False  # Allow during logout when auth may be revoked
    ...
    async def get(self, request: Request) -> Response:
        ...
        # Try to get the user, but don't fail if auth is already revoked
        user: User | None = request.get(KEY_HASS_USER)
        if not user:
            # User already logged out, but still return logout URL
            # so browser can be redirected to IdP logout page
            payload = {
